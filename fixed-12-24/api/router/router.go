...
func setApi_V1(engine *gin.Engine) {
	...
	login := engine.Group(ver + "/login")
	{
		...
		//	login.POST("/UpdateProxy", controller.UpdateProxyApi)
	}
	//equipment
	...
	message := engine.Group(ver + "/message")
	{
		...
		message.GET("/GetCDNDns", controller.GetCDNDnsApi)
	...
	{ //
		...
		user.POST("/BatchGetContactBriefInfo", controller.BatchGetContactBriefInfoApi)
